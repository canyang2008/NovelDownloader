...
def main():
    os.makedirs("data/Bookstore/Default/Default", exist_ok=True)
    os.makedirs("data/Record", exist_ok=True)
    os.makedirs("data/Record/User Data", exist_ok=True)
    if not os.path.exists("data/Record/urls.txt"):
        open('data/Record/urls.txt', 'w', encoding='utf-8').close()
    # 检查UrlConfig.json是否存在
    if os.path.exists("data/Record/UrlConfig.json"):
        ...
        init_urlconfig()
        print("data/Record/UrlConfig.json 不存在，已创建")
        with open("data/Record/UrlConfig.json", encoding='utf-8') as f:
            url_config = json.load(f)
    ...
    if url_config_version != '1.0.1':
        json_copy_to_root()
        url_config['Version'] = '1.0.1'
        url_config_format = json.dumps(url_config, ensure_ascii=False, indent=4)
        with open("data/Record/UrlConfig.json", 'w', encoding='utf-8') as f:
            f.write(url_config_format)

    # 检查SettingConfig.json是否存在
    if os.path.exists("data/Record/SettingConfig.json"):
        with open("data/Record/SettingConfig.json", encoding='utf-8') as f:
            try:
                ...
                print(
                    f"data/Record/SettingConfig.json  {Fore.LIGHTYELLOW_EX}不完整，请检查配置文件SettingConfig.json是否损坏")
                input()
    else:
        init_settingconfig()
        print("data/Record/SettingConfig.json 不存在，已创建")
        with open("data/Record/SettingConfig.json", encoding='utf-8') as f:
            setting_config = json.load(f)

    # 检查SettingConfig.json的版本
    ...
    # 检查template.html是否存在
    if not os.path.exists("data/Record/template.html"):
        init_template()
        print("data/Record/template.html不存在，已创建")


def json_copy_to_root():
    with open('data/Record/UrlConfig.json', 'r', encoding='utf-8') as f:
        url_config = json.load(f)
    if url_config['Version'] != '1.0.0':
        return True
    root_dir = os.path.join(os.getenv("LOCALAPPDATA"), "CyNovelbase", "data")
    all_urls = 0
    process_urls = 0
    for user in url_config.keys():
        ...
        for group in url_config[user].keys():
            for url in url_config[user][group]:
                all_urls += 1
                config = url_config[user][group][url]
                if config['Save_method']['json']['name'] == 'name_default':
                    file_name = config['Name'] + '.json'
                else:
                    file_name = config['Save_method']['json']['name'] if re.match(r"\.json$", config['Name']) else \
                    config['Save_method']['json']['name']
                file_path = os.path.join(config['Save_method']['json']['dir'], file_name)
                if os.path.exists(file_path):
                    ...
                    print(f"找不到通过url({url})下载的json文件，当更新小说时将会重头下载")
    url_config['Version'] = '1.0.1'
    with open("data/Record/UrlConfig.json", 'w', encoding='utf-8') as f:
        json.dump(url_config, f, ensure_ascii=False, indent=4)
    print(f"共有 {all_urls} 需处理，{process_urls}已完成:{process_urls}")
    return True


def init_settingconfig():
    with open("data/Record/SettingConfig.json", 'w', encoding='utf-8') as f:
        f.write(r"""{
      "Version": "1.0.0",
        "Default": {
          "Group": "Default",
          "User_data_dir": "data\\Record\\User Data",
          "Wait_time": [
                1,
                3
            ],
...
def init_template():
    with open("data/Record/template.html", 'w', encoding='utf-8') as f:
