...
## 更新记录
- ### 2026/03/07
  **版本： NovelDownloader-1.2.1**

  **修改**
  - 修复分组基本的创建、切换、删除问题
  - 修复账户基本的创建、切换、删除问题
  - 修复番茄的API和REQ模式下的下载问题  
  **添加**
    - Novel.total，表示所有章节数  
  <br>
  ps：bug还是有，只因时间不足，全天四分之一的时间都在修，没时间压力测试。当然最基本的下载功能（Chrome，不更新）还是可以使用的  
    接下来几个小版本会经历一些小改变，为了更好地自定义。自定义教程已经写了解析器的（教程应该是没人要的吧，又寸）  
    下次更新时间就是在节假日上了  
    手机终端模拟器支持？仍需进一步的测试  
    html和epub仍然有问题啊啊啊  
- ### 2026/02/05 - 2026/03/01
  **版本： NovelDownloader-1.2.0**

  **修改**  
  - 小说基本数据格式(不支持读取旧版小说数据)  
  - 更加少量的内存占用(尤其插图很多时)  
  - 获取的数据更加精确  
  **添加**  
  - 支持更多系统平台  
...
- ### 2026/02/28-2026/03/01
  ...
  - biquge.py的参数小适应
  - qidian.py的参数小适应
  - 初始化参数调整，防止浏览器用户数据目录跑到工作目录下
  - Novel.cover_img_data调整，初始化为b''
  - Group移除Last_download_time和first_download_time变量
  - txt.py支持gap和file_size参数，即分割操作
- ### 2026/02/22
  - 调整函数调用位置
- ### 2026/02/21
  - 完善部分保存逻辑
...
- ### 2026/02/20
  ...
  - 完善番茄和起点的搜索功能
- ### 2026/02/19
  - 修改base.py的保存逻辑，改为不省略地保留键名。
  - 修改storage.py的read_novel_data机制
  - 修改Group的add方法，更改为update方法，增加去重(group_name,url)机制
  - 修改main.py的update_novel机制，获取需要更新的章节序号。由于从oiapi的获取book_id有可能不准，所以不以url为准。
  - 增加Website的值:QIMAO
- ### 2026/02/05-18
  - 02/18:完善对于配置一的保存逻辑，修改add_save方法，避免因有无效参数而报错
  - 02/17:修复保存问题，基本可以运行
  - 基层构建且部分修改ds给予的架构
  - `__init__`
    - 添加add_save方法
    - 添加set方法，支持增量修改
  - core
    - Downloader去config中心化(从一个实例中获取参数)，不填写参数也可以创建Downloader。create_downloader返回Downloader列表，每个Downloader仅包含一个Tab|Session，以适应多线程
    - BaseDownloader去除抽象方法`download`
    - 完成storage.py的read_novel_data方法
  - models
    - 增加Website类以规范
    - 去除GlobalGroupConfig，GroupConfig修改为Group,增加属性和方法(find_group,add)
    - 去除Config，Novel.config改为`Novel.group`，去除`Novel.__add__`，改为`Novel.update`，添加`Novel.find_chapter`
    - 修改save.py的...（部分配置未应用）。"root"表达不准确，改为"base"
  - outputs
    - 添加抽象方法`Output.close`,`Output.__init__`
    - 完成base.py(小说基本数据)
    - 图片保存从json.py剥离，独立为img.py
    - url_parse函数返回`tuple[str,Website]`
  - parsers
    - 02/16:修改了大量逻辑，实现了多线程下载小说
    - 去config中心化(从一个实例中获取参数)。Perser类初始化必须填写参数Downloader
    - 统一打包下载配置
    - BaseParser添加抽象方法`parse_search_info`(搜索功能)，部分平台和部分下载模式支持搜索功能
    - 02/08:自主完成对番茄搜索文本乱码的破解
- ### 2026/01/04

  **版本：NovelDownloader-1.2.1-Beta.2**
...
- ### 2025/12/20
  ...
  **版本：NovelDownloader-1.2.0-Beta.1**

  **修改**
  - 将处理含有非法字符的功能迁移到此
  - 在同一程序下载多个小说时出现内容重叠到同一文件上
  **添加**
  - 可以在 下载 功能上通过章节标题关键字定义下载起始点，参数：URL "KEY"。如
  ```text
  https://fanqienovel.com/page/7276384138653862966 "戏鬼回家"
  ```
  ⚠️注意：下载同一小说时会覆盖原来的小说
- ### 2025/11/23
